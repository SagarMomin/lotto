...
module Common_options = struct
  ...
  let tickets_file =
    flag
      ...
      (required file)
      ~doc:
        "[FILE] A csv like file that has contains the all the numbers: in the \
         form of\n\n\
...
module Lottery = struct
  module Ticket = struct
    ...
    let t_of_int_list l =
      { white_balls = List.slice l 0 (-1) |> Int.Set.of_list
      ; magic = List.last_exn l
      }
    ;;
    ...
    let see_what_matches ~(winning_ticket : t) (t : t) =
      ...
      let { white_balls = winning_white_balls; magic = winning_magic } =
        winning_ticket
      in
      let num_match =
        Set.inter white_balls winning_white_balls |> Set.length
      in
...
let command =
  Command.group
    ...
    [ "megamillions", lotto ~lottery:Megamillions
    ; "powerball", lotto ~lottery:Powerball
    ]
;;
